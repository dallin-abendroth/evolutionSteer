...
public class evolutionSteer extends PApplet{
	...
	static boolean haveGround = true;
	int histBarsPerMeter = 5;
	String fitnessUnit = "chomps";
	String fitnessName = "Chomps";
	static final float baselineEnergy = (float) 0.0;
	static int energyDirection = 1; // if 1, it'll count up how much energy is used.  if -1, it'll count down from the baseline energy, and when energy hits 0, the creature dies.
	final static float FRICTION = 4;
	static float bigMutationChance = (float) 0.03;
	static float hazelStairs = -1;
	float cumulativeAngularVelocity = 0;
	boolean saveFramesPerGeneration = true;
	int gridBGColor = color(220, 253, 102, 255);
	static float foodAngleChange = (float) 0.0;
		
	
	static int nbCreatures = 100; // please set even number
	int gridX = 10; // X * Y must be equal to nbCreatures !
	int gridY = 10;
	...
	static float pressureUnit = (float) (500.0/2.37);
	static float energyUnit = 20;
	static float nauseaUnit = 5;
	int minBar = -10;
	int maxBar = 100;
	int barLen = maxBar-minBar;
	int gensToDo = 0;
	float postFontSize = (float) 0.96;
	static float scaleToFixBug = 1000;
	float energy = 0;
	float averageNodeNausea = 0;
	static float totalNodeNausea = 0;
	float CAMERA_MOVE_SPEED = (float) 0.03;
	
	float lineY1 = (float) -0.08; // These are for the lines of text on each node.
	float lineY2 = (float) 0.35;
	...
	float camZ = 0;
	float camHA = 0;
	float camVA = (float) -0.5;
	static int frames = 60;
	static int simDuration = 15; // in seconds
	static int jumperDuration = 1; // definition of jumper : chomp < this value (in seconds)
	static int maxFrames = simDuration*frames;
	int maxSimulationFrames = maxFrames;
	static int jumperFrames = jumperDuration*frames;
	static float giftForChompSec = 15;
	static int giftForChompFrames = ceil(giftForChompSec*frames);
	int menu = 0;
	int gen = -1;
	float sliderX = 1170;
	int genSelected = 0;
	...
	static Creature[] c = new Creature[nbCreatures];
	
	float camZoom = (float) 0.015;
	static float gravity = (float) 0.006;//0.007;
	static float airFriction =(float)  0.95;
	static float MIN_FOOD_DISTANCE = (float) 1;
	static float MAX_FOOD_DISTANCE = (float) 2.5;
	...
	int[] p = new int[29];
	final static int BRAIN_WIDTH = 3;
	static float STARTING_AXON_VARIABILITY = (float) 1.0;
	static float AXON_START_MUTABILITY = (float) 0.0005;
	...
	float radioactiveMutator = (float) 1.5;
	
	boolean massExtinction = false;
	
	static String[] patronData;
	static int PATRON_COUNT = 75;
	static float TOTAL_PLEDGED = (float) 183.39;
	static int[] CREATURES_PER_PATRON = new int[PATRON_COUNT];
	...
	int rInt() {
	  return (int)(random((float)-0.01, (float)1.01));
	}
	void drawGround(PGraphics img) {
	  ...
	  img.noStroke();
	  if (haveGround){
	    float groundTileSize = (float)5.0;
	...
	static float toMuscleUsable(float f){
		  return min(max(f,(float)0.8),(float)1.2);
	}
		void drawPosts(PGraphics img) {
		  ...
		  img.fill(0,0,255);
		  img.beginShape();
		  float s = (float)0.4*scaleToFixBug;
		  float y = (float)-0.001*scaleToFixBug;
		  img.vertex(0,y,s);
		  img.vertex(-s,y,0);
		  img.vertex(0,y,-s);
		  img.vertex(s,y,0);
		  ...
		  colorMode(HSB,(float)1.0);
		  int c = color((int)(timer%40)/40,1,1);
		  img.fill(c);
		  img.noStroke();
		  img.lights();
		  ...
		  img.sphere((float)0.4*scaleToFixBug*(float)0.5);
		  img.popMatrix();
		  ...
		  img.scale((float)1,(float)0.02,1);
		  img.sphere((float)0.4*scaleToFixBug*(float)0.5);
		  img.popMatrix();
		}
		void drawArrow(float x, float y, float z, PGraphics img) {
		  ...
		  img.fill(255, 0, 0);
		  img.beginShape();
		  float dist = (float)2.7*scaleToFixBug;
		  img.vertex(dist, (float)-3.8*scaleToFixBug, 0);
		  img.vertex(dist, (float)-2.7*scaleToFixBug, 0);
		  img.vertex(-dist, (float)-2.7*scaleToFixBug, 0);
		  img.vertex(-dist, (float)-3.8*scaleToFixBug, 0);
		  img.endShape();
		  img.beginShape();
		  img.vertex(0, (float)-2.2*scaleToFixBug);
		  img.vertex((float)-0.5*scaleToFixBug, (float)-2.7*scaleToFixBug);
		  img.vertex((float)0.5*scaleToFixBug, (float)-2.7*scaleToFixBug);
		  img.endShape(CLOSE);
		  String fitnessString = nf(currentCreature.getFitness(),0,2)+" "+fitnessUnit;
		  img.fill(255);
		  img.text(fitnessString, 0, (float)-2.91*scaleToFixBug,(float)0.1*scaleToFixBug);
		  img.popMatrix();
		}
		void drawGraphImage() {
		  ...
		  if (gen >= 1) {
		    stroke(0, 160, 0, 255);
		    strokeWeight(3);
		    float genWidth = (float)563.333333333/gen;
		    float lineX = (float)136.66666+genSelected*genWidth;
		...
		int getColor(int i, boolean adjust) {
			  colorMode(HSB, (float)1.0);
			  float col = (i*(float)1.618034)%1;
			  if (i == 46) {
			    col = (float)0.083333;
			  }
			  float light = (float)1.0;
			  if (abs(col-(float)0.333) <= 0.18 && adjust) {
			    light = (float)0.7;
			  }
			  int c = color((float)col, (float)1, (float)light);
			  colorMode(RGB, 255);
			  return c;
			}
			...
			void drawLines(int x, int y, int graphWidth, int graphHeight) {
			  ...
			  for (float i = ceil((worst-(best-worst)/(float)18.0)/unit)*unit; i < best+(best-worst)/(float)18.0;i+=unit) {
			...
			void drawSegBars(int x, int y, int graphWidth, int graphHeight) {
			  ...
			  segBarImage.background(0, (float)0, (float)0.5);
			  float genWidth = (float)(graphWidth)/gen;
			...
			void drawScreenImage(int stage) {
			  ...
			  screenImage.scale((float)10.0*windowSizeMultiplier/scaleToFixBug);
			  ...
			  for (int j = 0; j < nbCreatures; j++) {
			    ...
			    float xWidth = windowWidth / (gridX+1) / (float)10.0;
			    float yHeight = (windowHeight - gridHeightCrop)  / (gridY+1) / (float)10.0;
			    //if (stage >= 1) y++;
			    screenImage.pushMatrix();
			    screenImage.translate(((x+1)*xWidth)*scaleToFixBug, ((y+(float)0.5)*yHeight+(gridHeightCrop/(float)20.0))*scaleToFixBug, 0);
			  ...
			  if (stage == 0) {
			    ...
			    for (int j = 0; j < nbCreatures; j++) {
			      ...
			      if (cj.alive) {
			        ...
			        screenImage.vertex((x+(float)1)*xWidth-(float)15, (y+(float)0.5)*yHeight+(gridHeightCrop/(float)2)-(float)12,(float)0.01);
			        screenImage.vertex((x+(float)1)*xWidth+(float)15, (y+(float)0.5)*yHeight+(gridHeightCrop/(float)2)-(float)12,(float)0.01);
			        screenImage.vertex((x+(float)1)*xWidth+(float)15, (y+(float)0.5)*yHeight+(gridHeightCrop/(float)2)+(float)12,(float)0.01);
			        screenImage.vertex((x+(float)1)*xWidth-(float)15, (y+(float)0.5)*yHeight+(gridHeightCrop/(float)2)+(float)12,(float)0.01);
			...
			void drawpopUpImage() {
			  ...
			  float camDist = ((float)450/(float)2.0) / tan(PI*(float)30.0 / (float)180.0);
			  ...
			  popUpImage.scale((float)1.0/camZoom/scaleToFixBug);
			...
			void drawHistogram(int x, int y, int hw, int hh) {
			  ...
			  float multiplier = (float)hh/maxH*(float)0.9;
			  textAlign(LEFT);
			  textFont(font, 16);
			  stroke(128);
			  strokeWeight(2);
			...
			void drawStatusWindow(boolean isFirstFrame) {
			  ...
			  if (statusWindow >= 0) {
			    ...
			    px = floor((x+(float)1)*xWidth);
			    py = floor((y+(float)0.5)*yHeight+(gridHeightCrop/2)-19);
			    if (px <= 1140) {
			      px += 80;
			    } else {
			      px -= 80;
			    }
			    rect((x+1)*xWidth-15, (y+(float)0.5)*yHeight+(gridHeightCrop/(float)2)-(float)12, (float)30, (float)25);
			  } else {
			    cj = creatureDatabase.get((genSelected-1)*3+statusWindow+3);
			  ...
			  if (miniSimulation) {
			    ...
			    drawStats(px2+(float)355, py2+(float)239, (float)1, (float)0.45);
...
void initPercentiles(){
  for (int i = 1; i < 29; i++) {
    p[i] = (int)(floor((float)(pPercentages[i])*(float)(nbCreatures)/(float)1000.0));
  };
}
	public static void main(String[] args) {
        PApplet.main("evolutionSteer");
    ...
    public void draw(){
    	  ...
    	  if (menu == 0) {
    	    ...
    	    if (gen == -1) {
    	      ...
    	      textAlign(CENTER);
    	      textAlign(RIGHT);
    	      text((float)(round(percentile.get(min(genSelected, percentile.size()-1))[14]*nbCreatures))/nbCreatures+" "+fitnessUnit, (float)700, (float)160);
    	    ...
    	    screenImage.pushMatrix();
    	    screenImage.scale((float)10.0/scaleToFixBug);
    	    float xWidth = windowWidth / (gridX+1) / (float)10.0;
    	    float yHeight = (windowHeight - gridHeightCrop) / (gridY+1) / (float)10.0;
    	    for (int y = 0; y < gridY; y++) {
    	      for (int x = 0; x < gridX; x++) {
    	        screenImage.pushMatrix();
    	        screenImage.translate(((x+1)*xWidth)*scaleToFixBug, ((y+1)*yHeight+gridHeightCrop/(float)20.0)*scaleToFixBug, 0);
    	  ...
    	  if (menu == 5) { //simulate running
    	    ...
    	    if (timer <= maxFrames) {
    	      ...
    	      float camDist = (height/(float)2.0) / tan(PI*(float)30.0 / (float)180.0);
    	      simulationImage.pushMatrix();
    	      simulationImage.camera(camX/camZoom+camDist*sin(camHA)*cos(camVA),
    	      camY/camZoom+camDist*sin(camVA), camZ/camZoom+camDist*cos(camHA)*cos(camVA),
    	      camX/camZoom, camY/camZoom, camZ/camZoom, 0, 1, 0);

    	      simulationImage.scale((float)1.0/camZoom/scaleToFixBug);
    	      ...
    	      drawStats(windowWidth-(float)10,(float)0,(float)0,(float)0.7);
    	      drawSkipButton();
    	      drawOtherButtons();
    	    }
    	    if (timer == maxFrames) {
    	  ...
    	  if (menu == 8) {
    	    ...
    	    screenImage.background(gridBGColor);
    	    screenImage.pushMatrix();
    	    screenImage.scale((float)10.0/scaleToFixBug*windowSizeMultiplier);
    	    float transition = (float)0.5-(float)0.5*cos(min((float)(timer)/(float)60, PI));
    	    float xWidth = windowWidth / (gridX+1) / (float)10.0;
    	    float yHeight = (windowHeight - gridHeightCrop) / (gridY+1) / (float)10.0;
    	    for (int j = 0; j < nbCreatures; j++) {
    	      Creature cj = c2.get(j);
    	      ...
    	      screenImage.translate(((x3+(float)1)*xWidth)*scaleToFixBug, ((y3+(float)0.5)*yHeight-(gridHeightCrop/(float)2))*scaleToFixBug, 0);
    	      cj.drawCreature(screenImage,true);
    	    }
    	    screenImage.popMatrix();
    	    if (stepbystepslow) {
    	    ...
    	    for (int j = 0; j < nbCreatures/2; j++) {
    	      ...
    	      if(massExtinction && random(0,1) < 0.5){ // mass extinction
    	        ...
    	        if(enableRadioactivity && j >= nbCreatures/2 - freshBloodNumber) {
    	          ...
    	          c2.set(nbCreatures-1-j2, cj.modified(cj2.id+nbCreatures, (float)1.0));   //mutated offspring 1
    	        }
    	      }
    	    }
    	    for (int j = 0; j < nbCreatures; j++) {
    	  ...
    	  if (menu == 1 || gensToDo >= 1) {
    	    ...
    	    if (gen >= 1) {
    	      ...
    	      if (drag) {
    	    	  	...
    	    	  	sliderX = min(max(sliderX+(mX-25-sliderX)*(float)0.2, (float)760), (float)1170);
    	      }
    	      fill(100);
    	      rect(760, 340, 460, 50);
    	      fill(220);
    	      ...
    	      fill(0);
    	      text(genSelected, sliderX+(float)25, 366+fontSize*(float)0.3333);
    	    }
    	    if (genSelected >= 1) {
    	      simulationImage.beginDraw();
    	      ...
    	      for (int k = 0; k < 3; k++) {
    	        ...
    	        simulationImage.scale((float)60.0/scaleToFixBug);
    ...
    String percentify(float n){
    	...
    	void keysToMoveCamera(){
    	  ...
    	  camVA = min(max(camVA,-PI*(float)0.499),-PI*(float)0.001);
    	}
	  ...
	  if (key == CODED) {
    	...
    	void drawStats(float x, float y, float z, float size){
    	  ...
    	  text("X: "+nf(currentCreature.averageX/(float)5.0,0,2)+"", 0, 160);
    	  text("Y: "+nf(-currentCreature.averageY/(float)5.0,0,2)+"", 0, 192);
    	  text("Z: "+nf(-currentCreature.averageZ/(float)5.0,0,2)+"", 0, 224);
    	...
    	void drawBrain(float x, float y, float z, float size, Creature c){
    	  ...
    	  translate(x,y,z);
    	  scale(size);
    	  currentCreature.brain.drawBrain(size,currentCreature,this);
    	  popMatrix();
    	}
    	void drawSkipButton(){
    	  fill(0);
    	...
    	public int[] getNewCreatureName(){
    	  float indexOfChoice = random((float)0,TOTAL_PLEDGED); 
    	...
    	Creature createNewCreature(int index){
    	  ...
    	  for (int i = 0; i < nodeNum; i++) {
    	    n.add(new Node(random(-1, 1), random(-1, 1), random(-1, 1),
    	    		(float)0, (float)0,(float) 0, (float)0.4, random(0, 1), this)); //replaced all nodes' sizes with 0.4, used to be random(0.1,1), random(0,1)
    	  }
    	  for (int i = 0; i < muscleNum; i++) {
    	    ...
    	    float len = random((float)0.5,(float)1.5);
    	    m.add(new Muscle(tc1, tc2, len, random((float)0.015, (float)0.06), this));
    	  }
    	  float heartbeat = random(40, 80);
    	  return new Creature(null, index+1, new ArrayList<Node>(n), new ArrayList<Muscle>(m), 0, true, heartbeat, (float)1.0, null, null, this); 
    	}
    	      ...
    	      if(fieldName.equals("seed")){ SEED = p.getIntValue(); }
    	        ...
    	        while((token = p.nextToken()) != JsonToken.END_ARRAY){
    	          if (token == JsonToken.START_OBJECT){
    	            Creature creature = new Creature(new int[2], 0, new ArrayList<Node>(),  new ArrayList<Muscle>(), 0, false, 0, 0, null, new float[100][3], this);
